-- تربط المكافأة اختيارياً بصنف أو وجبة حقيقية من المنيو (بدل تكرار الاسم/الصورة/
-- قيمة الخصم يدوياً) — لو مربوطة، الخصم وقت الاستبدال = سعر الصنف/الوجبة الحالي
-- (يُحسب حياً بـ create_pos_order، يتحدّث تلقائياً لو تغيّر السعر بعدين). المكافآت
-- العامة (بدون ربط، خصم نقدي يدوي) تبقى تشتغل بنفس آليتها الحالية بلا تغيير.
alter table rewards
  alter column name drop not null,
  add column product_id uuid references products (id) on delete restrict,
  add column combo_id uuid references combos (id) on delete restrict,
  add constraint rewards_product_combo_at_most_one check (num_nonnulls(product_id, combo_id) <= 1);

create index rewards_product_id_idx on rewards (product_id) where product_id is not null;
create index rewards_combo_id_idx on rewards (combo_id) where combo_id is not null;

-- يوسّع create_pos_order: لو المكافأة مربوطة بصنف/وجبة، الخصم = سعرها الحالي
-- (بدل rewards.discount_amount المخزّن) — بقية الدالة بدون تغيير.
