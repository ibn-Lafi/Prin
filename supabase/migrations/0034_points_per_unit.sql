-- تحدد الإدارة كم نقطة ولاء يربحها العميل عند شراء كل صنف/وجبة (بدل قاعدة
-- ثابتة "1 ريال = 10 نقاط" على إجمالي الطلب) — بنفس فلسفة points_cost
-- بجدول rewards (قيمة يضبطها المدير يدوياً لكل عنصر). القيمة الافتراضية 0
-- للأصناف/الوجبات الجديدة (المدير يقرر صراحة)، لكن نُعبّي الأصناف والوجبات
-- الموجودة حالياً بمعادلة القاعدة القديمة (floor(price*10)) حتى لا ينقطع
-- اكتساب النقاط فجأة على الكتالوج الحالي بعد هذا التحديث.
alter table products add column points_per_unit int not null default 0 check (points_per_unit >= 0);
alter table combos add column points_per_unit int not null default 0 check (points_per_unit >= 0);

update products set points_per_unit = floor(price * 10) where deleted_at is null;
update combos set points_per_unit = floor(price * 10) where deleted_at is null;

-- يوسّع create_pos_order: النقاط المكتسبة الآن = مجموع (points_per_unit ×
-- الكمية) لكل صنف/وجبة بالطلب فعلياً، بدل نسبة ثابتة من الإجمالي — يُحسب
-- بنفس تمريرة إدراج order_items (نفس مكان قراءة السعر الحالي أصلاً)،
-- ومستقل عن أي خصم مطبّق (خصم المكافأة/كود الخصم يقلّل المبلغ المدفوع، مو
-- عدد النقاط المكتسبة من الأصناف المشتراة فعلياً). بقية الدالة بدون تغيير.
