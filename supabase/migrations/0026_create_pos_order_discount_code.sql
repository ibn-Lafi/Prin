-- يضيف تطبيق كود خصم لـ create_pos_order — يُطبَّق بعد خصم استبدال المكافأة
-- (تسلسلي: المكافأة أولاً على المجموع الفرعي الأصلي، ثم الكود على الناتج)،
-- بينما الحد الأدنى لقيمة الطلب (min_order_amount) يُتحقّق منه مقابل المجموع
-- الفرعي الأصلي قبل أي خصم — هذا هو المعنى المفهوم لـ"حد أدنى للطلب".
drop function if exists create_pos_order(uuid, text, text, jsonb, jsonb, uuid);
...
revoke all on function create_pos_order(uuid, text, text, jsonb, jsonb, uuid, text) from public;
grant execute on function create_pos_order(uuid, text, text, jsonb, jsonb, uuid, text) to service_role;
