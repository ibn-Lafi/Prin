...
function GroupForm({
  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl bg-[var(--color-brand-background)] p-3">
      ...
      <input
        type="text"
        placeholder="اسم المجموعة (مثال: نوع الخبز)"
        value={name}
...
function ModifierRow({
  ...
  function handleUpdate(input: ModifierInput) {
    ...
    startTransition(async () => {
      const result = await updateModifierAction(modifier.id, productId, input);
      if (result.error) {
  ...
  function handleToggle() {
    onError(null);
    startTransition(async () => {
      const result = await toggleModifierAvailabilityAction(modifier.id, productId, !modifier.is_available);
  ...
  function handleDelete() {
    onError(null);
    startTransition(async () => {
      const result = await deleteModifierAction(modifier.id, productId);
      if (result.error) onError(result.error);
    });
  }
...
function GroupCard({ group, productId }: { group: ModifierGroup; productId: string }) {
  ...
  function handleUpdateGroup(input: ModifierGroupInput) {
    setError(null);
    startTransition(async () => {
      const result = await updateModifierGroupAction(group.id, productId, input);
      if (result.error) {
  ...
  function handleDeleteGroup() {
    setError(null);
    startTransition(async () => {
      const result = await deleteModifierGroupAction(group.id, productId);
  ...
  function handleCreateModifier(input: ModifierInput) {
    setError(null);
    startTransition(async () => {
      const result = await createModifierAction(group.id, productId, input);
      if (result.error) {
        setError(result.error);
        return;
      }
    ...
    <div className="flex flex-col gap-3 rounded-2xl bg-[var(--color-brand-card)] p-4 ring-1 ring-[var(--color-brand-border)]">
      ...
      <div className="flex flex-col gap-1.5">
        {group.modifiers
          ...
          .map((modifier) => (
            <ModifierRow key={modifier.id} modifier={modifier} productId={productId} onError={setError} />
          ))}
      </div>
...
export function ModifierGroupsManager({
  ...
  function handleCreateGroup(input: ModifierGroupInput) {
    setError(null);
    startTransition(async () => {
      const result = await createModifierGroupAction(productId, input);
      if (result.error) {
        setError(result.error);
        return;
      }
